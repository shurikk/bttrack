...
describe "GET /announce" do
  ...
  describe "returns peers" do
    ...
    it "in compact form if asked" do
      get '/announce', info_hash: info_hash, peer_id: peer_id, compact: 1
      expect(response).to include('peers' => [
    ...
    it "excluding peer id if asked" do
      get '/announce', info_hash: info_hash, peer_id: peer_id, no_peer_id: 1
      expect(response['peers']).to have(3).items
      expect(response).to include('peers' => [
    ...
    it "limited to numwant if provided" do
      get '/announce', info_hash: info_hash, peer_id: peer_id, numwant: 2
      expect(response['peers']).to have(2).items
    end

    it "ignore zero or invalid numwant" do
      get '/announce', info_hash: info_hash, peer_id: peer_id, numwant: 0
      expect(response['peers']).to have(3).items
      get '/announce', info_hash: info_hash, peer_id: peer_id, numwant: -20
      expect(response['peers']).to have(3).items
      get '/announce', info_hash: info_hash, peer_id: peer_id, numwant: 'abc'
      expect(response['peers']).to have(3).items
    end
  end
  ...
  describe "stopped event" do
    context "with matching entry" do
      ...
      it "removes peer from list" do
        expect {
          get '/announce', info_hash: info_hash, peer_id: peer_id, event: 'stopped'
  ...
  describe "with started or completed event" do
    it "created peer entry" do
      expect {
        get '/announce', info_hash: info_hash, peer_id: peer_id, event: 'started', downloaded: 0, uploaded: 0, left: 512
      }.to change { store.get_peers.size }.by(1)
    ...
    it "updates peer entry" do
      store.set_peer peer_id, ip: '42.5.4.3', port: 432
      expect {
        get '/announce', info_hash: info_hash, peer_id: peer_id, event: 'completed', downloaded: 512, left: 0
  ...
  describe "peer identity protection (key)" do
    context "with previous key" do
      ...
      it "accepts request from authenticated client" do
        get '/announce', info_hash: info_hash, peer_id: peer_id, key: 'secret'
        expect(last_response).to be_ok
      end

      it "reject request from unknown client" do
        get '/announce', info_hash: info_hash, peer_id: peer_id, key: 'wrong'
        expect(last_response).to_not be_ok
    ...
    context "with no key set" do
      it "accepts any request if no key is set" do
        ...
        get '/announce', info_hash: info_hash, peer_id: peer_id, key: 'toto'
        expect(last_response).to be_ok
      end
